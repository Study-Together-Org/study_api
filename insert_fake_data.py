...
def generate_sorted_set():
    ...
    daily_timepoints = ["daily_" + str(day_timepoint - i * timedelta(days=1)) for i in range(60)]

    for sorted_set_name in daily_timepoints:

        to_insert = dict()
        for user_id in user_id_list: to_insert[user_id] = random.randint(0, 10)
        redis_client.zadd(sorted_set_name, to_insert)


    # week
    week_timepoint = utilities.get_week_start()
    weekly_timepoints = ["weekly_" + str(week_timepoint - i * timedelta(days=7)) for i in range(6)]

    for sorted_set_name in weekly_timepoints:

        to_insert = dict()
        for user_id in user_id_list: to_insert[user_id] = random.randint(0, 70)
        redis_client.zadd(sorted_set_name, to_insert)

    # month
    month_timepoint = utilities.get_month()
    to_insert = dict()
    for user_id in user_id_list: to_insert[user_id] = random.randint(0, 70 * 4)
